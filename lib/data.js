...
// Shared category taxonomy — stored in the DB as the slug, rendered via the
// label/hue. Keep popup and manager in sync by only ever reading this list.
const BEACONNEST_CATEGORIES = [
  { slug: "animation",  label: "Animation & Interaction", hue: 262 },
  { slug: "typography", label: "Typography",              hue: 32 },
  { slug: "layout",     label: "Layout & Grid",           hue: 200 },
  { slug: "color",      label: "Color & Style",           hue: 330 },
  { slug: "components", label: "Components & Sections",   hue: 152 },
  { slug: "general",    label: "General",                 hue: 222 },
];

function beaconnestCategory(slug) {
  return (
    BEACONNEST_CATEGORIES.find((c) => c.slug === slug) ||
    BEACONNEST_CATEGORIES[BEACONNEST_CATEGORIES.length - 1] // "general"
  );
}
...
async function beaconnestAddBeacon({ title, url, description, category, selector, selectedText, snippet, scrollX, scrollY, scrollYRatio, screenshotDataUrl, createdByName }) {
  const client = await beaconnestRequireClient();
...
function beaconnestToCSV(list) {
  const headers = ["title", "url", "description", "category", "selector", "selectedText", "scrollX", "scrollY", "createdByName", "createdByEmail", "createdAt", "updatedAt"];
