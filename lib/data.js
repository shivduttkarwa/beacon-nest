...
async function beaconnestAddBeacon({ title, url, description, selector, selectedText, snippet, scrollX, scrollY, screenshotDataUrl, createdByName }) {
  const client = await beaconnestRequireClient();
...
function beaconnestBuildTargetUrl(beacon) {
  try {
    const u = new URL(beacon.url);
    u.hash = "";
    let url = u.toString();
    if (beacon.selectedText) {
      url += `#:~:text=${encodeURIComponent(beacon.selectedText)}`;
    }
    return url;
  } catch (e) {
    return beacon.url;
