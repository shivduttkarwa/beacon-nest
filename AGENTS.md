...
| `fonts/` | Bundled Inter variable font (latin + latin-ext woff2, OFL license included) — loaded via `@font-face` in both `popup.css` and `manager.css`. Bundled locally because MV3's CSP for extension pages blocks font CDNs, same reason `lib/vendor/supabase.js` is vendored. |
